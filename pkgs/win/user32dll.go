...
var (
	...
	_SetWindowCompositionAttribute = user32dll.NewProc("SetWindowCompositionAttribute")
	_SetWindowDisplayAffinity      = user32dll.NewProc("SetWindowDisplayAffinity")
	_SystemParametersInfoW         = user32dll.NewProc("SystemParametersInfoW")
)
...
func SetWindowCompositionAttribute(hwnd HWND, data *WINDOWCOMPOSITIONATTRIBDATA) bool {
	ret, _, _ := _SetWindowCompositionAttribute.Call(hwnd, uintptr(unsafe.Pointer(data)))
	return ret != 0
}

func SetWindowDisplayAffinity(hwnd uintptr, affinity uint32) bool {
	ret, _, _ := _SetWindowDisplayAffinity.Call(hwnd, uintptr(affinity))
	return ret != 0
}
