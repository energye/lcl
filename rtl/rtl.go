...
const Separator = string(filepath.Separator) // 平台目录分隔符
...
func Combine(path, name string) string {
	if path != "" && !strings.HasSuffix(path, Separator) {
		path += Separator
	}
	if name != "" && strings.HasPrefix(name, Separator) {
		name = name[1:]
	}
	return path + name
}
