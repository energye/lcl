...
func MessageDlg(Msg string, DlgType TMsgDlgType, Buttons ...int32) int32 {
...
//
// 主线程中执行, 同步
func RunOnMainThreadSync(fn TMainThreadSyncProc) {
	api.DRunMainSyncCall(fn)
}
...
func FindControl(handle HWND) IWinControl {
	return AsWinControl(api.DFindControl(handle))
}

func FindLCLControl(screenPos TPoint) IControl {
	return AsControl(api.DFindLCLControl(screenPos))
}

func FindOwnerControl(handle HWND) IWinControl {
	return AsWinControl(api.DFindOwnerControl(handle))
}

func FindControlAtPosition(position TPoint, allowDisabled bool) IControl {
	return AsControl(api.DFindControlAtPosition(position, allowDisabled))
}

func FindLCLWindow(screenPos TPoint, allowDisabled bool) IWinControl {
	return AsWinControl(api.DFindLCLWindow(screenPos, allowDisabled))
}

func FindDragTarget(position TPoint, allowDisabled bool) IControl {
